...
const Card = ({ tasks, selectedDate }) => {
  ...
  const getDisplayDate = (task) => {
    // Если есть выбранная дата - показываем ее
    if (selectedDate) {
      return formatDate(selectedDate) || "Selected date";
    }

    // Иначе показываем дату задачи
    return formatDate(task.date) || "No date";
  };

  return (
    <Cards>
      {tasks.map((task) => (
        <Link to={`pop-browse/${task._id}`}>
          <CardItem key={task._id}>
            <CardWrapper>
              <CardGroup>
                ...
                <CardButton to={`pop-browse/${task._id}`}>
                  <div></div>
                  <div></div>
                  <div></div>
                </CardButton>
