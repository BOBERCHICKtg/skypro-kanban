...
const Card = ({ tasks, onTaskClick }) => {
  if (!tasks || tasks.length === 0) {
    return null; // Не отображаем ничего если нет задач
  }

  return (
    <Cards>
      {tasks.map((task) => (
        <CardItem key={task._id} onClick={() => onTaskClick(task)}>
          <CardWrapper>
            <CardGroup>
              <CardTheme
                color={
              ...
              <CardButton onClick={(e) => e.stopPropagation()}>
                <div></div>
                <div></div>
                <div></div>
              </CardButton>
